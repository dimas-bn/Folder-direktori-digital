(function(){
  ...
  var DATA = DATAKELAS_DATA; // [{kelas, siswa:[{no, nama}]}]
  ...
  // DATAKELAS_POIN didefinisikan di poin.js — dibuat opsional (guard) supaya
  // halaman tetap jalan normal walau poin.js belum/terlambat dimuat.
  var POIN_DATA = (typeof DATAKELAS_POIN !== 'undefined') ? DATAKELAS_POIN : {};
  ...
  function renderNetwork(){
    ...
    parts.push(
      '<text x="' + cx + '" y="' + (cy - 4) + '" text-anchor="middle" fill="#EAF2FF" font-family="Space Grotesk, sans-serif" font-weight="700" font-size="13">DATA</text>' +
      '<text x="' + cx + '" y="' + (cy + 13) + '" text-anchor="middle" fill="#5A6B8C" font-family="JetBrains Mono, monospace" font-size="10" letter-spacing="1">KELAS</text>'
    );
  ...
  function copyClassData(format){
    var text = buildCopyText(format);
    var doneLabel = format === 'nama' ? 'Nama Tersalin ✓' : 'Tersalin ✓';
    var done = function(){
    ...
    return (
      '<div class="print-block">' +
        '<div class="print-head">' +
          '<div class="print-brand">DATA KELAS<span class="print-brand__tag">.</span></div>' +
          '<div class="print-meta">Demo Sekolah<br>Informatika &middot; Koding dan Kecerdasan Artifisial (KKA)</div>' +
        '</div>' +
        '<h1 class="print-title">Daftar Siswa Kelas ' + escapeHtml(d.kelas) + '</h1>' +
        '<p class="print-sub">Tahun Ajaran Sekarang &middot; Dicetak ' + tanggal + avgText + '</p>' +
  ...
  function init(){
    ...
    copyMenu = document.getElementById('modal-copy-menu');
