# DATA KELAS — Direktori Kelas Digital (Demo)

Demo direktori data kelas ampuan untuk mata pelajaran **Informatika** serta
**Koding dan Kecerdasan Artifisial (KKA)** — dibuat sebagai pratinjau
sebagian fitur dari **Aplikasi Jurnal Mengajar Online** (versi berbayar).

Dibangun oleh **Dimas Bagus Nurdiansyah**.

> Seluruh nama siswa, kelas, dan data poin di project ini adalah **data
> contoh (dummy)** untuk keperluan demonstrasi — bukan data siswa
> sungguhan.
...
├── data.js         # data siswa per kelas (data contoh/dummy)
├── poin.js         # rekap poin siswa (data contoh, meniru fitur Rekap di Jurnal Mengajar Online)
├── manifest.json   # identitas PWA
├── sw.js           # service worker (offline)
├── pwa.js          # pop-up ajakan instalasi PWA
...
server, atau database. Semua data dimuat langsung dari `data.js` dan `poin.js`.

⚠️ **Semua file di atas wajib ikut ter-upload** setiap kali deploy ulang —
jangan hanya sebagian, karena `index.html` mereferensikan semuanya lewat
tag `<script>`. Kalau ada salah satu file yang tertinggal atau tertimpa versi
lama, fitur terkait file itu akan hilang dari tampilan meski datanya sendiri
masih aman tersimpan di repo.

## Rekap Poin (demo)

`poin.js` berisi rata-rata poin tiap siswa (data contoh), terpisah dari
`data.js` supaya mudah diperbarui manual tanpa menyentuh data induk. Isinya
tampil sebagai:
- kolom **"Poin"** di tabel detail siswa (dalam modal kelas)
- badge rata-rata poin di setiap kartu kelas (halaman utama)
- ikut tersalin lewat tombol **Salin Data**
- ikut tercetak di **Cetak / PDF** (plus rata-rata kelas di subjudul)
- ikut tampil di hasil **Cari Siswa** lintas kelas

**Cara update poin:** buka `poin.js`, cari kelas & nama siswa (harus persis
sama ejaannya dengan di `data.js`, besar-kecil huruf tidak masalah), lalu
ubah angkanya. Siswa yang belum diisi otomatis tampil sebagai "–". Skala
nilai default 0–7, meniru skala poin per pertemuan di Jurnal Mengajar Online.

## PWA (Progressive Web App)

DATA KELAS bisa dipasang seperti aplikasi biasa ke HP atau laptop.
...
- **Android / Chrome & Edge desktop** — begitu browser mendeteksi situs ini
  memenuhi syarat PWA, muncul banner "Pasang DATA KELAS di perangkat ini" di
...
ditekan "Nanti Saja"), buka: `https://<domain-deploy-kamu>/?resetpwa=1` —
status "pernah ditolak" akan otomatis terhapus dari perangkat itu. Cara ini
tidak akan menampilkan banner kalau situsnya memang **sudah terpasang**
sebagai aplikasi di perangkat tersebut — itu sudah sesuai desain, bukan bug
(untuk menguji dari kondisi benar-benar baru, coba buka lewat jendela
Incognito/Penyamaran, atau uninstall dulu aplikasinya).

**Offline:** setelah dibuka sekali secara online, `service worker` menyimpan
salinan halaman dan data siswa di perangkat, sehingga DATA KELAS tetap bisa
...
- Diagram jaringan animasi (berputar halus, searah jarum jam) menghubungkan
  kelima kelas ke inti DATA KELAS — tidak ada satu kelas yang menetap di
  posisi atas
- Detail siswa per kelas (nomor presensi + nama) dalam modal, lengkap
  dengan kotak pencarian di dalam kelas tersebut
- **Rekap Poin** — kolom poin per siswa (data contoh, meniru fitur Rekap di
  Jurnal Mengajar Online) di tabel detail kelas, plus badge rata-rata poin
  di tiap kartu kelas (lihat bagian Rekap Poin di atas)
...
- CTA "Hubungi via WhatsApp" di bagian Tentang, mengarah ke kontak untuk
  yang tertarik dengan Aplikasi Jurnal Mengajar Online
